...
if ($mosConfig_sef) {
	...
	if (in_array('content', $url_array)) {
		...
		*/
		...
		if ($juri != '' && $juri != '/' && false === stripos( $_SERVER['REQUEST_URI'], "index\.php" ) && false === stripos( $_SERVER['REQUEST_URI'], "index2\.php" ) && false === stripos( $_SERVER['REQUEST_URI'], "/\?" ) && $_SERVER['QUERY_STRING'] == '' ) {
			header( 'HTTP/1.0 404 Not Found' );
			require_once( $mosConfig_absolute_path . '/templates/404.php' );
			exit( 404 );
		}
...
function sefRelToAbs( $string ) {
	...
	if( $mosConfig_sef && $mosConfig_multilingual_support && $string!='index.php' && !preg_match("@^(([^:/?#]+):)@i",$string) && !strcasecmp(substr($string,0,9),'index.php') && false === stripos($string, 'lang=') ) {
