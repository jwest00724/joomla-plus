...
function showPathway( $Itemid ) {
	...
	while ($i--) {
		if (!$mid || empty( $mitems[$mid] ) || $Itemid == $home_menu->id || false === stripos($optionstring, "option")) {
			break;
		}
		$item =& $mitems[$mid];
		...
		if (($item->id == $Itemid && !$mainframe->getCustomPathWay()) || empty( $mid ) || empty($item->link)) {
			...
			$correctLink = false !== stripos($item->link, 'http://');
			if ($correctLink==1) {
	...
	if ( false !== stripos( $optionstring, 'option' ) && trim( $path  ) ) {
		$home = '<a href="'. sefRelToAbs( 'index.php' ) .'" class="pathway">'. $home .'</a>';
	}
