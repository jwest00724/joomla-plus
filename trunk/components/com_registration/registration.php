...
function sendNewPass( $option ) {
	...
	global $_MAMBOTS;
	$_MAMBOTS->loadBotGroup('syncsystems');
	$_MAMBOTS->trigger( 'onLostPassword', array($database, $user_id, $checkusername, $clearpassword) );
...
function saveRegistration() {
	...
	global $_MAMBOTS;
	$_MAMBOTS->loadBotGroup('syncsystems');
	$_MAMBOTS->trigger( 'onUserSave', array($database, true, $row, '', $clearpassword) );
	...
	if ($mosConfig_useractivation == 1){
		$message = sprintf (_USEND_MSG_ACTIVATE, $name, $mosConfig_sitename, $mosConfig_live_site."/index.php?option=com_registration&task=activate&activation=".$row->activation, $mosConfig_live_site, $username, $clearpassword);
	} else {
