...
class mosInstaller {
	...
	var $i_elementdir 		= '';
	/** @var string The name of the J-One-Plus element */
	var $i_elementname 		= '';
	...
	*/
	function findInstallFile() {
		...
		if (count( $files ) > 0) {
			...
			$this->setError( 1, 'ERROR: Could not find a J-One-Plus XML setup file in the package.' );
			return false;
		} else {
	...
	*/
	function readInstallFile() {
		...
		if ($root->getTagName() != 'mosinstall') {
			$this->setError( 1, 'File :"' . $this->installFilename() . '" is not a valid J-One-Plus installation file' );
			return false;
		}
