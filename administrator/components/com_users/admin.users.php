...
function editUser( $uid='0', $option='users' ) {
	...
	if ( $my_group == 'super administrator' && $my->gid != 25 ) {
		...
		if (is_array( $my_groups ) && count( $my_groups ) > 0) {
			$ex_groups = (array) $acl->get_group_children( $my_groups[0], 'ARO', 'RECURSE' );
		} else {
			$ex_groups = array();
		}
...
function saveUser( $task ) {
	...
	if ($isNew) {
		// new user stuff
		if ($row->password == '') {
			$clearpassword 			= mosMakePassword();

			$salt = mosMakePassword(16);
			$crypt = md5($clearpassword.$salt);
	...
	*/
	...
	$_MAMBOTS->trigger( 'onUserSave', array($database, $isNew, $row, (isset($original->username) ? $original->username : ''), $clearpassword) );
	...
	if ($isNew) {
		...
		$message = sprintf ( _NEW_USER_MESSAGE, $row->name, $mosConfig_sitename, $mosConfig_live_site, $row->username, $clearpassword );
