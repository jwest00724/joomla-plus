...
class mosMainFrame {
	...
	var $_isAdmin 					= false;
	/** @var array An array of menu row objects */
	var $_menus						= array();
	...
	*/
	function mosMainFrame( $db, $option, $basePath, $isAdmin=false ) {
		...
		$this->_head = array();
		$this->_head['title'] 	= ('com_frontpage' == $option AND !empty($GLOBALS['mosConfig_MetaHomeTitle'])) ? $GLOBALS['mosConfig_MetaHomeTitle'] : $GLOBALS['mosConfig_sitename'];
		...
		$this->set( 'now', $now );
		$this->_db->setQuery("SELECT * FROM #__menu");
		$this->_menus = $this->_db->loadObjectList('id');
	}

	function getMenuID ($type, $link, $published=true) {
		foreach ($this->_menus as $menu) {
			if ($published AND !$menu->published) continue;
			if ($type == $menu->type AND $link == $menu->link) return $menu->id;
		}
		return 0;
	}

	function getMenuCount ($type, $published=true) {
		$count = 0;
		foreach ($this->_menus as $menu) {
			if ($published AND !$menu->published) continue;
			if ($type != $menu->type) continue;
			$count++;
		}
		return $count;
	}
	...
	*/
	function setPageTitle( $title=null ) {
		if (@$GLOBALS['mosConfig_pagetitles']) {
			if (@$GLOBALS['mosConfig_MetaHomeTitle'] != $this->_head['title']) {
				$title = trim( htmlspecialchars( $title ) );
				$title = stripslashes($title);
				if ($title) $alltitles[] = $title;
			}
			if (!empty($this->_head['title'])) $alltitles[] = $this->_head['title'];
			if (isset($alltitles)) $this->_head['title'] = implode(' | ', $alltitles);
		}
	}
	/**
  ...
  /**
	...
	*/
	function initSession() {
		...
		$this->_session 	= new mosSession( $this->_db );
	...
	*/
	function login( $username=null, $passwd=null, $remember=0, $userid=NULL ) {
		...
		if (!$username || !$passwd) {
			...
			// extra check to ensure that J-One-Pluas sessioncookie exists
		...
		if (!$username || !$passwd) {
			...
			if (is_object($row)) {
				...
				$this->_session->username 	= $row->username;
				$this->_session->userid 	= intval( $row->id );
				$this->_session->usertype 	= $row->usertype;
				$this->_session->gid 		= intval( $row->gid );
				...
				if ( $_VERSION->SITE ) {
					...
					$query = "DELETE FROM #__session"
					. "\n WHERE session_id != ". $this->_db->Quote( $this->_session->session_id )
				...
				. "\n WHERE id = " . (int) $this->_session->userid
				;
				$this->_db->setQuery($query);
				if (!$this->_db->query()) {
	...
	function getItemid( $id, $typed=1, $link=1, $bs=1, $bc=1, $gbs=1 ) {
		...
		if ($_Itemid == '' && $typed && $this->getStaticContentCount()) {
			...
			if ( !$exists ) {
				...
				$ContentTyped[$id] 	= $this->getMenuID('content_typed', 'index.php?option=com_content&task=view&id=' . (int) $id);
		...
		if ($_Itemid == '' && $link && $this->getContentItemLinkCount()) {
			...
			if ( !$exists ) {
				...
				// add query result to temp array storage
				$ContentItemLink[$id] 	= $this->getMenuID('content_item_link', 'index.php?option=com_content&task=view&id=' . (int) $id);
				// save temp array to main array storage
	...
	*/
	function getStaticContentCount( ) {
		...
		if ( !$this->get( '_StaticContentCount' ) && !defined( '_JOS_SCC' ) ) {
			...
			$this->set( '_StaticContentCount', $this->getMenuCount('content_typed') );
		}
	...
	*/
	function getContentItemLinkCount( ) {
		...
		if ( !$this->get( '_ContentItemLinkCount' ) && !defined( '_JOS_CILC' ) ) {
			 ...
			 */
			// saves query result to variable
			$this->set( '_ContentItemLinkCount', $this->getMenuCount('content_item_link') );
		}
